...
class Trainer():
 ...
 # learning rate scheduler, step
  ...
  def load(self):
    ...
    if os.path.isfile(os.path.join(model_path, 'latest.ckpt').replace(" ","")):
      latest_epoch = open(os.path.join(model_path, 'latest.ckpt').replace(" ",""), 'r').read().splitlines()[-1]
    else:
      ckpts = [os.path.basename(i).split('.pth')[0] for i in glob.glob(os.path.join(model_path, '*.pth'))]
      ckpts.sort()
      latest_epoch = ckpts[-1] if len(ckpts)>0 else None
    if latest_epoch is not None:
      gen_path = os.path.join(model_path, 'gen_{}.pth'.format(str(latest_epoch).zfill(5))).replace(" ","")
      dis_path = os.path.join(model_path, 'dis_{}.pth'.format(str(latest_epoch).zfill(5))).replace(" ","")
      opt_path = os.path.join(model_path, 'opt_{}.pth'.format(str(latest_epoch).zfill(5))).replace(" ","")
