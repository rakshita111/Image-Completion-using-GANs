...
class Dataset(torch.utils.data.Dataset):
  def __init__(self, data_args, debug=False, split='train', level=None):
    ...
    self.data = [os.path.join(data_args['zip_root'], data_args['name'], i).replace("\\","/")
      for i in np.genfromtxt(os.path.join(data_args['flist_root'], data_args['name'], split+'.flist').replace("\\","/"), dtype=np.str, encoding='utf-8')]
    self.mask_type = data_args.get('mask', 'pconv')
    if self.mask_type == 'pconv':
      self.mask = [os.path.join(data_args['zip_root'], 'mask/{}.png'.format(str(i).zfill(5))).replace("\\","/") for i in range(2000, 12000)]
      if self.level is not None:
        self.mask = [os.path.join(data_args['zip_root'], 'mask/{}.png'.format(str(i).zfill(5))).replace("\\","/") for i in range(self.level*2000, (self.level+1)*2000)]
  ...
  def load_item(self, index):
    ...
    img = ZipReader.imread(img_path, img_name) 
    # load mask 
    if self.mask_type == 'pconv':
