...
class InpaintGenerator(BaseNetwork):
  def __init__(self, init_weights=True):#1046
    ...
    self.at_conv02 = AtnConv(cnum*2, cnum*2, fuse=False)
    self.at_conv01 = AtnConv(cnum, cnum)

    # attention fuse
    self.at_fuse_05 = AtnFuse(cnum * 16)
    self.at_fuse_04 = AtnFuse(cnum * 8)
    self.at_fuse_03 = AtnFuse(cnum * 4)
    self.at_fuse_02 = AtnFuse(cnum * 2)
  ...
  def forward(self, img, mask):
    ...
    x2 = self.at_fuse_02(x2, self.gamma)
    ...
    # upx2 = self.at_fuse_02(upx2, self.gamma)
...
class AtnFuse(nn.Module):
  """Attention fuse model in coarse-to-fine order with dilation rates 3,5,7"""
  def __init__(self, input_channels):
    super(AtnFuse, self).__init__()

    # atrous convolutions
    self.atr_conv_r7 = nn.Sequential(
      nn.Conv2d(input_channels, input_channels, kernel_size=3, stride=1, padding=7 , dilation=7),
      nn.LeakyReLU(0.2, inplace=True))
    self.atr_conv_r5 = nn.Sequential(
      nn.Conv2d(input_channels, input_channels, kernel_size=3, stride=1, padding=5 , dilation=5),
      nn.LeakyReLU(0.2, inplace=True))
    self.atr_conv_r3 = nn.Sequential(
      nn.Conv2d(input_channels, input_channels, kernel_size=3, stride=1, padding=3 , dilation=3),
      nn.LeakyReLU(0.2, inplace=True))
    self.atr_conv_r1_3 = nn.Sequential(
      nn.Conv2d(input_channels, input_channels, kernel_size=3, stride=1, padding=1 , dilation=1),
      nn.LeakyReLU(0.2, inplace=True))
    self.atr_conv_r1_1 = nn.Sequential(
    ...
    self.conv_block1 = nn.Sequential(
      ...
      nn.LeakyReLU(0.2, inplace=True),
      nn.Conv2d(input_channels*4, 1, kernel_size=3, stride=1, padding=1),
      nn.Sigmoid())
  ...
  def forward(self, x, gamma):
    ...
    x1 = self.atr_conv_r7(x)
    x2 = self.atr_conv_r5(x)
    x3 = self.atr_conv_r3(x)
    x4 = self.atr_conv_r1_3(x)
    x5 = self.atr_conv_r1_1(x)

    xa1 = torch.cat([x1, x2], dim=1)
    a1 = self.conv_block1(xa1)
    xo1 = x1*a1 + x2*(1-a1)
    xa2 = torch.cat([xo1, x3], dim=1)
    a2 = self.conv_block1(xa2)
    xo2 = x2*a2 + x3*(1-a2)
    xa3 = torch.cat([xo2, x4], dim=1)
    a3 = self.conv_block1(xa3)
    xo3 = x3 * a3 + x4 * (1 - a3)
    xa4 = torch.cat([xo3, x5], dim=1)
    a4 = self.conv_block1(xa4)
    xo4 = x4 * a4 + x5 * (1 - a4)
    xo = gamma*xo4 + (1-gamma)*x

    xo = self.conv_block2(xo)
